...
- **Fixed Shift+Enter inserting duplicate newlines in Claude Code** — Shift+Enter was inserting a bracketed-paste newline _and_ a raw carriage return, which made Claude Code submit in-progress messages early. The fix calls `preventDefault` on every intercepted keystroke (Ctrl+F, Escape, Ctrl+C, Shift+Enter, Ctrl+Shift+Up/Down), matching VS Code's xterm integration pattern. (#57)
