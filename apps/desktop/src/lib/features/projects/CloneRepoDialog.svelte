<script lang="ts">
	...
	let filteredRepos = $derived.by(() => {
		...
		return repos.filter(
			(r) =>
				r.nameWithOwner.toLowerCase().includes(q) || (r.description ?? '').toLowerCase().includes(q)
		);
	});
	...
	<Dialog.Content class="flex max-h-[90vh] max-w-2xl flex-col overflow-hidden">
		...
		<div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto">
			...
			<div class="flex flex-col gap-2">
				...
				{:else if reposError}
					<div class="space-y-2 rounded-md border border-destructive/40 bg-destructive/10 p-3">
						<p class="text-xs font-medium text-destructive">Failed to load repositories</p>
						<p class="font-mono text-[11px] break-all text-muted-foreground">{reposError}</p>
					...
					<ScrollArea class="h-48 rounded-md border">
						<div class="divide-y">
							{#each filteredRepos as repo (repo.nameWithOwner)}
								<button
									class="flex w-full items-center gap-2 px-3 py-2 text-left text-xs transition-colors hover:bg-muted/50 {selectedRepo?.nameWithOwner ===
									repo.nameWithOwner
										? 'bg-accent'
										: ''}"
									type="button"
								...
								>
									<div class="min-w-0 flex-1">
										...
										{#if repo.description}
											<span class="ml-1.5 truncate text-muted-foreground">{repo.description}</span>
										{/if}
									</div>
