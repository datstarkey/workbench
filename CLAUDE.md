...
- `src/lib/stores/claudeSessions.svelte.ts` — Discovers Claude sessions from CLI JSONL files (no custom persistence)
...
- `discover_claude_sessions` Rust command reads JSONL files — used both for the resume menu and to detect session IDs for newly launched sessions (polled every 2s after launch).
- Session IDs are never generated by the frontend; they come from the JSONL files created by the CLI.
- `claudeSessions.svelte.ts` is a thin wrapper around `discover_claude_sessions` (no custom persistence).
