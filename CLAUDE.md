...
- Not covered by automated tests: the desktop GUI (WebDriver, macOS-unsupported), the `terminal.rs` attach lock-ordering reattach race (timing-only), and the `spawn_blocking` offloading (behavior-preserving).
