...
- **Android (mobile):** `cd apps/mobile && bun run tauri android build --apk --target aarch64` (`--debug` for debug; `bun --cwd apps/mobile run tauri …` does not work). Needs `ANDROID_HOME`, `NDK_HOME`, `JAVA_HOME` (JDK 21). Launcher icons: `bunx tauri icon app-icon/manifest.json` from `apps/mobile` (adaptive foreground/background SVGs live in `app-icon/`).
- **Server (standalone):** `cargo run -p workbench-server -- --port 4317 --token <32+ chars>` (flags: `--bind`, `--port`, `--token`, `--insecure-no-token`; envs `WORKBENCH_BIND/PORT/TOKEN`). Refuses to start on a non-loopback bind without a token unless `--insecure-no-token`; a token shorter than 32 chars is always refused.
...
- **Layer 2 — Rust commands via the mock runtime.** `tauri::test` (`mock_builder().manage(state).build(mock_context(noop_assets()))`) drives `#[tauri::command]`s with no webview. Enabled by `tauri = { features = ["test"] }` declared as a **dev-dependency** (`apps/desktop/src-tauri/Cargo.toml`) so feature unification keeps `test` out of release builds. Example: `server_control.rs`'s `lan_start_status_stop_cycle` exercises the start/stop/status commands (a repeat start with the same token is a no-op; a different token restarts); `rotating_the_token_restarts_a_running_lan_listener` asserts the old token is rejected after rotation; `listeners_share_terminals_across_lan_restarts` proves both listeners see one terminal set. Thin wrapper commands (`commands.rs` etc.) stay untested at this layer — their logic lives in `workbench-core` (unit-tested).
- **Layer 3 — WebDriver e2e: smoke only** (`apps/desktop/e2e`). WebdriverIO's `@wdio/tauri-service` with the **embedded** provider: `tauri-plugin-wdio-webdriver` runs a W3C WebDriver server inside the app, so it works on macOS (WKWebView), where `tauri-driver` doesn't. The plugin is behind the opt-in cargo feature `e2e` and a `compile_error!` blocks it in release: it's an unauthenticated server on `127.0.0.1:4445`, and sandboxed sessions can reach loopback, so it must never be in `tauri dev` or shipped builds. `bun run --cwd apps/desktop build:e2e` (`tauri build --debug --no-bundle --features e2e`) then `test:e2e`. `wdio.conf.ts` points `WORKBENCH_CONFIG_DIR` at a temp dir seeded with the repo as the only project. Specs check boot, sidebar, the Scripts tab and the settings window; the `e2e` CI job (Ubuntu + xvfb) is not a required check yet. `tauri-plugin-wdio` (mocking/log capture) is not installed: its `execute` permission runs arbitrary JS.
- **Rust (`cargo test`):** `workbench-core` units, the `workbench` desktop crate units (`pty`, dispatchers, `server_control` mock-runtime), `workbench-server` units, and `apps/server/tests/server.rs` (e2e: real server on an ephemeral port, driven with reqwest + a `tokio-tungstenite` WS client). The e2e suite covers spawn→list→kill, idempotent `remote_kill` (204 not 500), unregistered-cwd rejection, session/terminal caps, bearer auth (no/wrong token → 401 on HTTP and WS, weak token refused), foreign-`Origin` WS rejection (403), two listeners sharing `Managers`, and WS-close on kill **and** on shell exit. Every embedded test server needs a ≥32-char token (`TOKEN` const + `client()` / `ws_url()` helpers).
...
- **Deliberately not unit-tested — this is the WebDriver layer's job:** rendered `.svelte` component interaction. Our components are thin context-coupled views that delegate to the thoroughly-tested stores, and they wrap Bits UI / shadcn primitives (dialog portals, focus traps) that are friction-prone and flaky under jsdom. So the policy is: **push real logic out of components into stores/helpers and unit-test that** (e.g. `MobileClient`, `terminalWsUrl`), rather than render-test the views. Rendering is smoke-checked by the WebDriver layer; keep new component behaviour in stores/helpers anyway.
...
- `server_control.rs` — `ServerControl` owns ONE `workbench_server::Managers` (terminals + remote-control sessions) shared by two listeners: the always-on **loopback** listener (desktop xterm panes; per-process in-memory token from `workbench_core::token::generate`, handed to the webview only via `terminal_server_status`, never persisted or put in a shell env) and the opt-in **LAN** listener (`start_server`/`stop_server`/`server_status`; `rotate_server_token`). Terminals therefore show on the phone and vice versa, and survive LAN restarts. The LAN slot remembers its token: `start_server` with a different token restarts it, and `rotate_server_token` is the one rotation path (loads settings from disk, replaces only `serverToken`, saves, restarts a running listener on its bind/port, emits `settings:changed`) so unsaved form edits never ride along.

### Server crate (`apps/server/src/`)

`workbench-server` (lib + bin). axum control plane; **no tauri**.

- `lib.rs` — `app()` (router + auth + CORS), `serve()` (standalone, own managers), `spawn_embedded(bind, port, Managers, token: String)`/`ServerHandle` (desktop; token is mandatory and must pass `token::is_strong`, so an embedded listener can't be unauthenticated). **`ServerHandle::stop()` revokes**: each listener's `AppState.revoked` watch flips, and every terminal WS attached through it gets `{"t":"revoked"}` + close — upgraded sockets outlive axum's graceful shutdown and the token is only checked at upgrade, so without this a phone kept typing after server mode was turned off or the token rotated. The loopback listener has its own signal; terminals stay alive for other listeners. Clients show closed/ended (not "Take control") on `revoked`. CORS allows any origin but only `Authorization`/`Content-Type` and no credentials mode — safe only because auth is a bearer header, never a cookie.
...
- `auth.rs` — bearer-token middleware (`/`, `/health` and the terminal WS path exempt — the WS checks `?token=` itself; constant-time compare; no-op only for a tokenless standalone server) + `ws_origin_allowed`: the terminal WS upgrade rejects any `Origin` other than the Tauri webviews (`tauri://localhost`, `http(s)://tauri.localhost`), the server's own page (origin == `Host`), or — debug builds only — the Vite dev servers on 1420/1430. Native clients send no `Origin` and pass.
- `error.rs` (`ApiError`: anyhow→HTTP), `state.rs` (`AppState`, `Managers`), `cli.rs` (clap + `resolved_token` start-up guard)
- Binds `0.0.0.0` with a required token; traffic is plaintext HTTP, so still keep it on a private network (Tailscale). `GET /settings/workbench` redacts `serverToken`, and spawned shells get `WORKBENCH_TOKEN` stripped from their env.
...
**Cross-device terminals.** The loopback server's terminal list includes terminals opened from other devices (shared managers). `TerminalAdoptionPoller` (`features/terminal/server-terminals.ts`, started in `App.svelte` only after `workspaceStore.load()`) polls every 5s while visible + on window focus and calls `WorkspaceStore.adoptServerTerminal`, which adds a background tab to the matching xterm workspace (worktree, then main; never native, never creates workspaces) with `serverTerminalIds` preset. `terminal-connection.ts` **claims** every id this webview created/attached/killed and `listServerTerminals` returns null while a create is in flight — otherwise a pane's own PTY (listed before its id reaches the store) or a just-killed one would be adopted as a duplicate. The server allows ONE attacher: a new attach sends `{"t":"takeover"}` to the old socket. Adopted panes (`WorkspaceStore.adoptedPaneIds`) are excluded from the persisted snapshot (`withoutPanes` — their loopback PTYs die with the app, so they'd reopen as phantom shells; after a reload they're simply re-adopted), and closing one only detaches: no `DELETE` (it's the other device's session) and its id is released so it isn't re-adopted. They mount **detached** (`connectDetached`) and a taken-over pane stays detached, both showing "Take control" (`TerminalConnection.takeControl`) — never auto-reattach, or two devices ping-pong. Mobile-created PTYs have no `WORKBENCH_PANE_ID`/hook socket, so hook-based activity tracking only covers desktop-created panes. Panes send a readable `name` (`paneDisplayName`) alongside `paneId`.
...
- **`window.confirm` is broken under Tauri and cannot be fixed with a capability.** `tauri-plugin-dialog` injects an init script into every webview that replaces `window.confirm` with an async `invoke('plugin:dialog|confirm')` — but the plugin only registers `open`/`save`/`message`, so that command does not exist and no permission grants it (`dialog:allow-confirm` is a deprecated alias for `allow-message`). Any bare `confirm()` therefore rejects with "not allowed by ACL", and its return value is a Promise (always truthy) even when it doesn't. Never call `confirm()`; use `ConfirmDialog`/`ConfirmAction`. This is why `TerminalPane` must pass an explicit `linkHandler` to `new Terminal()` — xterm core's OscLinkProvider falls back to `confirm()` + `window.open()` for OSC 8 hyperlinks, which silently swallowed every such link click.
- **Never spawn a child process with `Command::new` — use `workbench_core::shell::command()`.** Release builds link `windows_subsystem = "windows"`, so the app owns no console and Windows pops a visible blank console window for every console child (`git`, `gh`, `taskkill`, …). `shell::command` sets `CREATE_NO_WINDOW`. Same module owns `default_shell()` / `login_args()` / `inherited_env()` — the per-platform PTY shell setup shared by the desktop `PtyManager` and the server's `TerminalManager` (`-l` is Unix-only; passing it to `cmd.exe`/`powershell.exe` fails the spawn).
