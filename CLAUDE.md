...
- `paths.rs` (`atomic_write`), `session_utils.rs`, `shell_integration.rs`, `token.rs` (CSPRNG bearer tokens + `is_strong`, ≥32 chars), `net.rs` (`pairing_addresses`: IPv4 via `if-addrs`, Tailscale 100.64/10 first, then RFC 1918)
